...
- **Built:** Research docs (`research/`: viability, tech stack, skills inventory, agent
  architecture). Claude Code scaffolding: `.claude/CLAUDE.md`, 10 subagent definitions in
  `.claude/agents/`, 26 skills in `.claude/skills/` (all per research/skills.md except
  `run-local`, which gets generated by /run-skill-generator once the app exists).
- **In progress:** Nothing active. Next planned step: repo scaffold (git init + directory
  structure) + Webull OpenAPI application + paper-trading harness.
- **Known issues / debt:** No PRD doc beyond the research files. Webull OpenAPI application
  not yet submitted (1–2 day approval — submit early). Not yet a git repository (also means
  agent/skill files aren't version-controlled yet). Safety "never" rules exist only as
  prompts — must become PreToolUse hooks / permissions.deny before live trading.
